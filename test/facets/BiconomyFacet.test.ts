import { BiconomyFacet } from '../../typechain'
...
describe('BiconomyFacet', function () {
  let lifi: BiconomyFacet
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      lifi = <BiconomyFacet>(
        await ethers.getContractAt('BiconomyFacet', diamond.address)
      )
      ...
      const router = await ethers.getContract('LiquidityPoolManager')
      await router.addSupportedToken(
        usdt.address,
        0,
  ...
  it('starts a bridge transaction on the sending chain', async () => {
    ...
    await lifi
      .connect(alice)
      .startBridgeTokensViaBiconomy(lifiData, BiconomyData, {
        gasLimit: 500000,
      })
  })
  ...
  it('performs a swap then starts bridge transaction on the sending chain', async () => {
    ...
    await lifi
      .connect(alice)
      .swapAndStartBridgeTokensViaBiconomy(lifiData, swapData, BiconomyData, {
        gasLimit: 500000,
